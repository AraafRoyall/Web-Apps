/* Secure ECIES3 Core
 * ECI4 v2
 * X25519 + HMAC-SHA256 KDF + AES-GCM
 * URL-safe Base64
 *
 * Keep this file stable. UI/layout changes belong in secure_ecies3_final.html.
 */
(function(global){
  "use strict";
  ...
  const derBitString = bytes =>
    derTag(0x03, concat(new Uint8Array([0]), bytes));
  const derOctet = bytes => derTag(0x04, bytes);

  const spkiFromRaw = raw =>
    derSeq(derSeq(derOid(OID_X25519)), derBitString(raw));

  const pkcs8FromRaw = raw =>
    derSeq(
      derInt(0),
      derSeq(derOid(OID_X25519)),
      derOctet(derOctet(raw))
    );
  ...
  const readDer = (buf, offset) => {
    ...
    const x = readDerLength(buf, offset + 1);
    return {
      tag,
      start:x.next,
      end:x.next + x.len,
  ...
  const rawFromSpki = spki => {
    const seq = readDer(spki, 0);
    const alg = readDer(spki, seq.start);
    const bit = readDer(spki, alg.next);

    if (seq.tag !== 0x30 || alg.tag !== 0x30 || bit.tag !== 0x03) {
  ...
  const rawFromPkcs8 = pkcs8 => {
    const seq = readDer(pkcs8, 0);
    const version = readDer(pkcs8, seq.start);
    const alg = readDer(pkcs8, version.next);
    const key = readDer(pkcs8, alg.next);
    ...
    const inner = pkcs8.slice(key.start, key.end);
    const oct = readDer(inner, 0);
  ...
  const deriveAesKey = async shared => {
    ...
    return (await hmac(prk, KDF_INFO)).slice(0, 32);
  };
  ...
  const encrypt = async (plainText, recipientPublicB64) => {
    ...
    const cipher = await aesGcmEncrypt(
      te.encode(plainText || ""),
      aesKey,
      iv
    );
  ...
  const decrypt = async (cipherB64, privateB64) => {
    const all = fromBase64Url(cipherB64);
    const privateRaw = fromBase64Url(privateB64);

    if (privateRaw.length !== RAW_LEN) {
    ...
    if (
      ...
      ephPublic = all.slice(5, 37);
      iv = all.slice(37, 49);
      cipher = all.slice(49);
    } else {
      if (all.length < 61 || all[0] !== 1) {
        throw new Error("Invalid legacy payload");
      }
      ephPublic = all.slice(1, 33);
      iv = all.slice(33, 45);
      cipher = all.slice(45);
    }

    if (
      ephPublic.length !== RAW_LEN ||
      iv.length !== IV_LEN ||
      cipher.length < 16
    ) {
    ...
    const ephKey = await importPublic(ephPublic);
    const privateKey = await importPrivate(privateRaw);

    const shared = new Uint8Array(
      await crypto.subtle.deriveBits(
        {name:"X25519",public:ephKey},
        privateKey,
        256
      )
    );
    ...
    const aesKey = await deriveAesKey(shared);
    const plain = await aesGcmDecrypt(cipher, aesKey, iv);
    return td.decode(plain);
  };

  global.ECIES3Core = Object.freeze({
    VERSION,
    RAW_LEN,
    IV_LEN,
    bytesToB64Url:toBase64Url,
    b64UrlToBytes:fromBase64Url,
    generateKeyPair,
    encrypt,
    decrypt
  });
